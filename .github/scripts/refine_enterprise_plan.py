...
html = html.replace('<div class="row"><span>批量发行与批量任务</span><span>标准批量</span><span>高级批量</span><span>高级批量 / API 自动化</span></div>', '<div class="row"><span>批量发行</span><span>—</span><span>✓</span><span>✓</span></div><div class="row"><span>API 自动化发行</span><span>随 API</span><span>随 API</span><span>✓</span></div>')
...
assert '标准批量' not in index_path.read_text()
assert '高级批量' not in index_path.read_text()
assert 'API 自动化发行' in index_path.read_text()
